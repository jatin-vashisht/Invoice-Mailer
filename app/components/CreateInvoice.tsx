    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardContent className="p-6">
        <form id={form.id} action={action} onSubmit={form.onSubmit} noValidate>
          {/* we can't give name, key, and defaultValue to the Calendar component, so we're making a hidden input element for the validations, giving the state to this component */}
          <input
            type="hidden"
            name={fields.date.name}
            value={selectedDate.toISOString()}
          />
          {/* As we don't have any state for the total, so we're making another hidden input to keep track of the state, as we've total defined in our invoice schema */}
          <input
            type="hidden"
